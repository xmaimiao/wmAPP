...
class BasePage:
    ...
    def wait_for_display(self,by,locator,timeout=10):
        ...
        stutas = WebDriverWait(self.driver, timeout).until(expected_conditions.visibility_of_element_located((by,locator)))
        if stutas:
    ...
    def tap(self, x_num,y_num):
        logging.info(f"tap：{x_num,y_num}")
        action = TouchAction(self.driver)
        window_rect = self.driver.get_window_rect()
        width = window_rect['width']
        height = window_rect['height']
    ...
    def set_implicitly_wait(self,second):
        logging.info(f"set_implicitly_wait：{second}")
